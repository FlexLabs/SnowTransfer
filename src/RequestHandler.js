...
class RequestHandler {
    constructor(options) {
        ...
        this.userAgent = `DiscordBot (Dyno API, ${version})`;
        this.ratelimits = {};
        this.latencyRef = {
            latency: 500,
            offset: 0, // client.options.ratelimiterOffset,
        ...
        return new Promise((resolve, reject) => {
            ...
            var actualCall = (cb) => {
                ...
                var req = HTTPS.request({
                    method: method,
                    host: this.options.baseHost || "discordapp.com",
                ...
                req.once("response", (resp) => {
                    ...
                    if(this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
                        ...
                        if(this.latencyRef.timeOffset - this.latencyRef.latency >= this.latencyThreshold && timeOffset - this.latencyRef.latency >= this.latencyThreshold) {
							console.error(`Your clock is ${this.latencyRef.timeOffset}ms behind Discord's server clock. Please check your connection and system time.`);
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.statusCode !== 429) {
							console.log(`${body && body.content} ${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[route].remaining}/${this.ratelimits[route].limit} left | Reset ${this.ratelimits[route].reset} (${this.ratelimits[route].reset - now}ms left)`);
